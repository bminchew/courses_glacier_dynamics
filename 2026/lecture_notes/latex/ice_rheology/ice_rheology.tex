...
\medskip
\noindent\textbf{How $n$ controls stability.}  Changing $n$ changes both the exponent $\gamma$ in equation~\eqref{eq:schoof_flux} and the prefactor $\Lambda$ (through its dependence on $A$).  When $n$ and $A$ are coupled through the composite flow law, the combined effect can qualitatively change the stability picture.  \cite{RanganathanMinchew2024} showed that for realistic bed geometries and accumulation rates (Figure~\ref{fig:misi}):
\begin{itemize}
  \item For $n = 3$ with the conventionally calibrated $A$: an unstable steady state exists on the retrograde bed slope.  MISI is possible.
  \item For $n \approx 1.8$ (GBS regime) with the corresponding higher $A$: the grounding line flux is sufficiently large that the flux curve stays above the accumulation line across the entire retrograde section.  No unstable steady state exists in this idealized configuration.
  \item For $n \approx 4$ (dislocation creep regime) with the corresponding lower $A$: the grounding line flux is too low to reach the accumulation line on the retrograde section.  Again, no unstable steady state exists in this configuration.
\end{itemize}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig10_misi_stability.pdf}
  \caption{\textbf{(A)} Idealized bed geometry with a retrograde section (shaded).  \textbf{(B)} Grounding line flux $q_g(x_g)$ for three values of $n$ with corresponding $A$ values from the composite flow law, plotted against the accumulation line $\dot{b}\,x_g$.  Intersections are steady-state grounding line positions; filled circles are stable, open circles are unstable.  For $n = 3$ (blue), an unstable steady state exists on the retrograde bed---this is the marine ice sheet instability (MISI).  For $n = 1.8$ (green) and $n = 4$ (red), all steady states are stable: no MISI occurs for this bed geometry and accumulation rate.  Following the analysis of \cite{RanganathanMinchew2024}, adapted from \cite{Schoof2007}.}
  \label{fig:misi}
\end{figure}

\noindent The conclusion is striking: our assessment of whether a marine ice sheet is vulnerable to unstable retreat depends on the assumed value of $n$.  The canonical assumption of $n = 3$ predicts MISI for a bed geometry that is unconditionally stable under both the GBS-dominated ($n \approx 1.8$) and dislocation-dominated ($n \approx 4$) regimes of the composite flow law.  Since the composite flow law predicts that the effective $n$ varies spatially across an ice sheet (\S\ref{sec:composite}), the stability of any given grounding line depends not just on the bed geometry and climate forcing, but on the local rheological regime---which is itself determined by the stress, temperature, and grain size.
...
\textit{Journal of Geophysical Research}, 112, F03S28.
\texttt{doi:10.1029/2006JF000664}
