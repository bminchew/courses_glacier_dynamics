...
\noindent Weertman idealized the bed as an inclined plane studded with cubical bumps of side length $a$, spaced a distance $\lambda$ apart.  Figure~\ref{fig:regelation_schematic} illustrates both mechanisms.
...
\begin{equation}
  ...
  \label{eq:separatio\phi_pressure}
\end{equation}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig07_cavity_formation.pdf}
...
\begin{figure}[ht]
  ...
  \caption{Stability of the undrained plastic bed (UPB) model, computed for Ice Stream B parameters ($\tau_d = 13$~kPa, $G = 0.06$~W\,m$^{-2}$, $\Theta_b = 0.04$~K\,m$^{-1}$).  \textbf{(a)} Basal melt rate as a function of bed strength.  The maximum shear heating occurs at $\tau_b = \tau_d / (n+1) = 0.25\,\tau_d$.  Two equilibria exist where $m_r = 0$: the ice stream mode (stable, filled circle) at low $\tau_b$ and the ice sheet mode (linearly unstable, open circle) at high $\tau_b$.  Arrows show the direction of evolution: melting weakens the bed (leftward) and freezing strengthens it (rightward).  \textbf{(b)} Corresponding ice stream velocity.  After \cite{Tulaczyketal2000b}, Figure~6.}
