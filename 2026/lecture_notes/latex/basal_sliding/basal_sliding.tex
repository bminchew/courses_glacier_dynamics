...
\medskip
\noindent Numerically, for $k_r = 3$~W\,m$^{-1}$\,K$^{-1}$ (granite), $\rho = 917$~kg\,m$^{-3}$, $L = 3.34 \times 10^5$~J\,kg$^{-1}$, $\tau_b = 100$~kPa, $R = 0.05$, and $a = 1$~mm:
\begin{equation*}
  u_1 = \frac{3 \times 7.4 \times 10^{-8}}{917 \times 3.34 \times 10^5 \times 10^{-3}} \times \frac{10^5}{0.05^2} \approx 2.9\;\text{mm\,s}^{-1} \approx 90\;\text{km\,yr}^{-1}.
\end{equation*}
For $a = 1$~m, the same calculation gives $u_1 \approx 0.09$~m\,yr$^{-1}$---negligible.  Regelation is rapid, and thus not rate-controlling, for millimeter-scale bumps and negligibly slow for meter-scale bumps.
...
Solving for $a_c^2$:
\begin{equation}
  a_c^2 = \frac{k_r\,\mathcal{B}}{\rho\,L\,A}\;\frac{\tau_d^{1-n}}{R^{2-2n}}\,(2R^2)^n
...
\begin{enumerate}
  ...
  \item \textbf{Ice sheet mode} (stable): $\tau_b > 0.25\,\tau_d$.  The bed is strong, the ice stream is slow or stagnant, and the base is frozen.  If a perturbation weakens the bed, shear heating increases (because we are on the right side of the maximum, where $\partial(\tau_b U_b)/\partial \tau_b > 0$), which increases melting, further weakening the bed---a positive feedback that would push the system toward the ice stream mode.  However, if the geothermal flux is insufficient to maintain melting at any bed strength in this range, the bed remains frozen and stable.
\end{enumerate}
