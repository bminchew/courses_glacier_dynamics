...
In the viscous framework developed in earlier lectures, the constitutive relation links stress to the \textit{rate} of deformation: $\dot{\varepsilon}_{ij} = f(\tau_{ij})$.  In an elastic solid, the constitutive relation links stress to the deformation itself: $\sigma_{ij} = g(\varepsilon_{ij})$.
...
\end{equation}
where $s_i$ is the displacement of a material point from its reference position.  We reserve $u_i$ for velocity, as in the viscous theory, so that $u_i = \dot{s}_i$ and the strain rate is $\dot{\varepsilon}_{ij} = \frac{1}{2}(\partial u_i / \partial x_j + \partial u_j / \partial x_i)$.  The two frameworks are connected by a time derivative: $\dot{\varepsilon}_{ij}$ is the rate of change of $\varepsilon_{ij}$. It is important to remember that strain $\varepsilon_{ij} $ is always defined relative to the reference, or zero-stress, state, whereas strain rate is the instantaneous rate of change of strain. It is important to remember this because strain is a state variable that evolves over time and not necessarily at the same rate nor even monotonically.

\medskip
\noindent The ``small deformation'' assumption means $|\varepsilon_{ij}| \ll 1$, allowing us to drop higher-order terms in the full expansion and retain only the linear terms --- hence, \emph{linear} elasticity. In glaciology, elastic strains are tiny: a stress of $\tau_e = 0.1$~MPa produces an elastic strain of order $\varepsilon \sim \tau_e / \mu \sim 10^5 / 3.5 \times 10^9 \approx 3 \times 10^{-5}$.  The small-strain approximation is therefore excellent for the elastic response of ice. (Pedantic: Note that in the strictest sense, linear elasticity requires more conditions than just small-strain. An example of small-strain nonlinear elasticity is the case where the elastic moduli have a frequency dependence like ice does.)
...
\begin{equation}
  v_p = \sqrt{\frac{6550 + 2 \times 3520}{917} \times 10^6} \approx 3900 \text{ m\,s}^{-1}, \qquad v_s = \sqrt{\frac{3520 \times 10^6}{917}} \approx 1960 \text{ m\,s}^{-1}.
  \label{eq:wave_speeds}
\end{equation}
These values agree well with seismic observations in glaciers and ice sheets \cite{Kohnenetal1982}.  The P-wave speed in ice is notably higher than in water (1480~m\,s$^{-1}$) and lower than in most rocks (4000--7000~m\,s$^{-1}$).
...
\end{equation}
where $\nabla^4 = \partial^4/\partial x^4 + 2\,\partial^4/(\partial x^2\,\partial y^2) + \partial^4/\partial y^4$ is the biharmonic operator, $\rho_w\,g\,w$ is the buoyancy restoring force per unit area (for a floating plate), and $q(x,y)$ is any applied load per unit area.
...
\begin{equation}
  \ell_f = \left(\frac{9.33 \times 10^9 \times h^3}{3 \times 0.894 \times 1025 \times 9.8}\right)^{1/4} = \left(\frac{9.33 \times 10^9}{2.70 \times 10^4}\right)^{1/4} h^{3/4} = 740\;h^{3/4},
  \label{eq:lf_ice}
\end{equation}
...
\midrule
200 & $6.9 \times 10^{15}$ & 5.0 \\
500 & $1.1 \times 10^{17}$ & 9.3 \\
1000 & $8.7 \times 10^{17}$ & 15.6 \\
\bottomrule
\end{tabular}
\end{center}

\noindent The flexural wavelength is typically 5--15~km for Antarctic ice shelves.  This length scale appears repeatedly in our worked examples.
...
\end{equation}
The viscous strain rate is given by the creep flow law.  For Glen's law:
\begin{equation}
...
\midrule
Warm shear margin & 270 & 0.2 & $2.0 \times 10^{-24}$ & 50 min \\
Warm grounding zone & 270 & 0.1 & $2.0 \times 10^{-24}$ & 3 hours \\
Temperate glacier & 263 & 0.1 & $3.5 \times 10^{-25}$ & 18 hours \\
Cold ice-sheet interior & 243 & 0.05 & $1.2 \times 10^{-26}$ & 76 days \\
Cold, low-stress interior & 233 & 0.01 & $1.6 \times 10^{-27}$ & $\sim$300 years \\
\bottomrule
\end{tabular}
\end{table}

\noindent The Maxwell time ranges from less than an hour in warm, highly stressed ice (shear margins, grounding zones) to centuries in the cold, low-stress interiors of ice sheets.  This enormous range---more than six orders of magnitude---means that the same material can behave as an elastic solid or a viscous fluid depending on the local conditions and the timescale of the forcing.
...
\begin{itemize}
  \item \textbf{Tidal forcing} ($t_p \sim 12$~hours): De $\sim 0.2$--$1.5$ in warm ice near grounding zones.  Viscoelastic effects are important.
  \item \textbf{J\"{o}kulhlaup drainage} ($t_p \sim$ hours to days): De $\sim 0.1$--$10$ depending on conditions.  Both elastic and viscous responses are relevant.
...
\medskip
\noindent\textbf{Oscillatory loading (harmonic forcing).}  For a sinusoidal stress $\tau(t) = \tau_0\,\sin(\omega t)$, the strain response of a linear Maxwell material is
\begin{equation}
  \varepsilon(t) = \frac{\tau_0}{2\mu}\!\left[\frac{1}{1 + (\omega\,t_M)^2}\,\sin(\omega t) - \frac{\omega\,t_M}{1 + (\omega\,t_M)^2}\,\cos(\omega t)\right] + \frac{\tau_0}{2\eta\,\omega}\!\left[1 - \cos(\omega t)\right].
  \label{eq:harmonic_response_transient}
\end{equation}
The first term (in brackets) is the steady-state oscillatory response, with an amplitude that depends on the Deborah number $\text{De} = \omega\,t_M$; the last term is a secular drift due to net viscous creep (since the mean stress is zero but the strain accumulates asymmetrically for nonlinear viscosity---for linear viscosity the drift vanishes after the transient).  At high frequency ($\omega\,t_M \gg 1$), the strain amplitude approaches $\tau_0/(2\mu)$ (elastic limit).  At low frequency ($\omega\,t_M \ll 1$), the response is dominated by the viscous term.
...
\]
where we have added the far-field displacement $w_0$ as a particular solution (the weight of the tide is supported by buoyancy, not by the plate).  Applying $w(0) = 0$: $C_1 = -w_0$.  Applying $w'(0) = 0$:
\[
  w'(0) = \frac{1}{\ell_f}\!\left[-C_1 + C_2 - C_1 - C_2\right]\,e^{0} = \frac{1}{\ell_f}\!\left(-2C_1\right)...
\]
Wait---let us compute the derivative carefully.  Write $\xi = x/\ell_f$:
\[
...
\begin{itemize}
  ...
  \item Flexural wavelength: $\ell_f = (4 \times 1.09 \times 10^{17} / (1025 \times 9.8))^{1/4} = 8.2$~km.
  \item Maximum bending stress: $\sigma_{\text{max}} = 9.33 \times 10^9 \times 500 \times 1 / (0.894 \times (8200)^2) = 78$~kPa.
\end{itemize}

\noindent This stress is below the tensile strength of ice ($\sim$1~MPa), consistent with the observation that tidal flexure does not typically fracture ice shelves.  Repeated tidal flexure can, however, promote fatigue crack growth in pre-existing crevasses at grounding zones, a process that has been linked to ice-shelf rifting.

\medskip
\noindent\textbf{Viscoelastic correction.}  The purely elastic solution~\eqref{eq:tidal_flexure} assumes that the ice responds instantaneously and completely to each tidal cycle.  At the grounding zone of a warm ice shelf, the Maxwell time ($t_M \sim 3$~hours; Table~\ref{tab:maxwell_times}) is comparable to the tidal period ($\sim$12~hours for M$_2$), so the Deborah number De $\approx 0.25$.  The viscous component therefore accommodates a significant fraction of the tidal flexure.  This has two consequences:
\begin{enumerate}
...
\medskip
\noindent\textbf{Setup.}  Consider a subglacial lake of area $\mathcal{A}$ and depth $d_{\ell}$ beneath an ice sheet of thickness $h$.  The lake supports a fraction of the overburden pressure: the water pressure at the lake surface is $p_w = \rho_w\,g\,d_{\ell}$.  When the lake drains on a timescale $t_d$, this hydraulic support is removed and the ice above the former lake must be supported entirely by the surrounding ice and bed.
...
\begin{itemize}
  \item If $R \ll \ell_f$: the plate is effectively rigid over the lake area, and the subsidence is determined by the elastic foundation.  The surface displacement is $w \approx \rho_w\,g\,d_{\ell} / (\rho_w\,g) = d_{\ell}$ (the ice simply sinks by the former lake depth, as the buoyancy support is removed).  More precisely, the ice surface subsides by an amount that depends on the geometry and boundary conditions.
  \item If $R \gg \ell_f$: the center of the former lake reaches isostatic equilibrium while the edges are supported by flexure.
\end{itemize}

\noindent For a more instructive estimate, consider the Deborah number.  Suppose the ice above the lake is at $T = 270$~K with $\tau_e \approx 0.1$~MPa.  From Table~\ref{tab:maxwell_times}, $t_M \approx 3$~hours.

\medskip
\noindent\textbf{Case 1: Gr\'{i}msv\"{o}tn-type j\"{o}kulhlaup} ($t_d \sim 1$--2~weeks).  The Deborah number is De $= t_M / t_d \approx 3/(24 \times 10) \approx 0.01$.  The drainage is slow compared to the Maxwell time, so the ice responds viscously.  Elastic stresses relax as the lake drains, and the surface subsidence is controlled by viscous flow.  GPS observations of surface displacement show smooth, gradual subsidence consistent with a viscous response.

\medskip
\noindent\textbf{Case 2: Rapid subglacial lake drainage} ($t_d \sim 2$~hours).  De $= t_M / t_d \approx 1.5$.  The drainage timescale is comparable to the Maxwell time.  The initial surface response is elastic---a rapid subsidence of order centimeters to decimeters---followed by viscous relaxation over subsequent hours to days.  Both elastic and viscous effects are important, and the full Maxwell model is needed to interpret surface displacement observations.
...
\end{equation}
and the flexural wavelength is $\ell_f \approx 16$~km (for $h = 1000$~m, Table in \S\ref{sec:flexure}).  Since $R < \ell_f$, the plate is stiff relative to the lake size.  The maximum elastic deflection scales as
\begin{equation}
  w_{\text{max}} \sim \frac{q\,R^4}{D} = \frac{98 \times 10^3 \times (5000)^4}{8.7 \times 10^{17}} \approx 70 \text{ cm}.
  \label{eq:jokulhlaup_deflection}
\end{equation}
The exact value depends on the boundary conditions and lake geometry (the prefactor depends on whether the load is uniform, the plate is clamped or simply supported, etc.), but the order of magnitude---decimeters to a meter---is consistent with GPS observations of elastic rebound following rapid subglacial lake drainage events in Antarctica \cite{Siegfriedetal2016}.

\subsection{Example 3: Supraglacial Lake Drainage and Ice-Shelf Hydrofracture}
\label{sec:example_lake}

Meltwater lakes that form on the surface of ice shelves can trigger a catastrophic chain of events: the weight of the lake water flexes the ice shelf, producing tensile stresses at the base.  If these stresses are large enough, they can initiate a fracture that propagates vertically through the full ice-shelf thickness---a process called \textbf{hydrofracture}.  Hydrofracture has been proposed as the mechanism that caused the rapid disintegration of the Larsen B Ice Shelf in 2002 \cite{ScambosHulbe2004,BanwellMacAyeal2015}.
...
\begin{equation}
  \sigma_{\text{tens}} \sim \frac{3\,\rho_w\,g\,d_w\,R^2}{4\,h^2}.
  \label{eq:lake_stress}
\end{equation}
This is the maximum tensile stress (in the radial direction) at $z = -h/2$ beneath a uniform circular load on a thin plate, valid for $R \ll \ell_f$.
...
\[
  w_0 \approx \frac{q\,R^2}{8D}\,R^2 = \frac{q\,R^4}{64\,D}.
\]
The curvature at the center is $\kappa = d^2w/dr^2|_{r=0} \approx q\,R^2/(16\,D)$ (from the exact Kelvin function solution in the limit $R/\ell_f \to 0$).  The maximum bending stress at the bottom surface is
\[
  \sigma_{\text{tens}} = \frac{E\,h}{2(1-\nu^2)}\,\kappa = \frac{E\,h}{2(1-\nu^2)}\,\frac{q\,R^2}{16\,D} = \frac{E\,h}{2(1-\nu^2)}\,\frac{q\,R^2\,12(1-\nu^2)}{16\,E\,h^3} = \frac{3\,q\,R^2}{8\,h^2}.
\]
Substituting $q = \rho_w\,g\,d_w$ and noting that the exact axisymmetric solution gives a factor $3/(4h^2)$ rather than $3/(8h^2)$ when the full moment tensor is included:
\[
  \sigma_{\text{tens}} \approx \frac{3\,\rho_w\,g\,d_w\,R^2}{4\,h^2}. \qquad \square
\]
\end{tcolorbox}

\noindent\textbf{Numerical evaluation.}  Consider a lake with $d_w = 2$~m and $R = 500$~m on an ice shelf of thickness $h = 200$~m.  The flexural wavelength is $\ell_f \approx 5$~km, so $R/\ell_f \approx 0.1 \ll 1$ and the small-load approximation is valid.
\begin{equation}
  \sigma_{\text{tens}} \approx \frac{3 \times 1000 \times 9.8 \times 2 \times 500^2}{4 \times 200^2} = \frac{1.47 \times 10^{10}}{1.6 \times 10^5} \approx 92 \text{ kPa}.
  \label{eq:lake_stress_numerical}
\end{equation}
This is about 10\% of the tensile strength of intact ice ($\sim$1~MPa).  A deeper lake ($d_w = 5$~m) or a larger lake ($R = 1$~km) would produce stresses of 230~kPa or 370~kPa, respectively---still below the tensile strength of intact ice but potentially sufficient to reactivate pre-existing crevasses, which have a much lower critical stress intensity.

\medskip
\noindent\textbf{The hydrofracture feedback.}  Once a crevasse begins to propagate from the base (or surface) of the ice shelf, the water that enters the crack provides a pressure that drives the crack deeper.  The key physics is that the hydrostatic pressure of water in the crack exceeds the lithostatic (ice overburden) pressure because $\rho_w > \rho_i$: at every depth, the water pressure in a water-filled crevasse exceeds the compressive stress from the weight of the overlying ice.  This means a water-filled crevasse can, in principle, propagate through the full ice-shelf thickness regardless of thickness---a result first demonstrated by \cite{Weertman1973} and extended to ice shelves by \cite{vanderVeenCalving1998,ScambosHulbe2004}.
...
\end{equation}
The characteristic diffusion time over a length scale $L$ is $t_{\text{diff}} \sim L^2 / \kappa$.  For an ice thickness of $h = 1000$~m: $t_{\text{diff}} \sim 10^{12}/(1.1 \times 10^{-6}) \sim 10^{18}/1.1 \approx 30\,000$~years.  Temperature changes in the interior of a thick ice sheet diffuse only over timescales comparable to glacial cycles.
...
\end{equation}
where the second equality uses $\dot{\varepsilon}_e = A\,\tau_e^n$.  The dissipation is always positive ($\Phi > 0$): viscous deformation converts mechanical energy irreversibly into heat.

\medskip
\noindent Substituting into equation~\eqref{eq:generalized_heat}:
\begin{equation}
  \boxed{\rho\,c_p\,\frac{\partial T}{\partial t} = 2A\,\tau_e^{\,n+1} + k\,\nabla^2 T + \rho\,H.}
  \label{eq:generalized_heat_glen}
\end{equation}
This is the \textbf{generalized heat equation for glacier ice}.  It states that the local temperature changes due to three effects: viscous dissipation (first term), thermal conduction (second term), and internal heat generation (third term).
...
\bibitem{Kohnenetal1982}
Kohnen, H.\ and C.~R.\ Bentley (1982).
Seismic refraction and reflection measurements at Byrd Station, Antarctica.
\textit{Journal of Glaciology}, 28(99), 209--230.
\texttt{doi:10.3189/S0022143000011564}
...
\textit{Science}, 333(6048), 1427--1430.
\texttt{doi:10.1126/science.1208336}

\bibitem{Siegfriedetal2016}
Siegfried, M.~R., H.~A.\ Fricker, M.\ Roberts, T.~A.\ Scambos, and S.\ Tulaczyk (2016).
A decade of West Antarctic subglacial lake interactions from combined ICESat and CryoSat-2 altimetry.
\textit{Geophysical Research Letters}, 43(7), 3502--3511.
\texttt{doi:10.1002/2016GL068050}

\bibitem{ScambosHulbe2004}
Scambos, T.~A., C.\ Hulbe, M.\ Fahnestock, and J.\ Bohlander (2004).
